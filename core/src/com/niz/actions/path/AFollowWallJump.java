...
import com.niz.actions.AWallSlide;
...
public class AFollowWallJump extends Action {
	private static final String TAG = "follow walljump action";
	...
	public AFollowWallJump() {
		lanes = Action.LANE_PATH;
		isBlocking = true;
	}
	@Override
	public void update(float dt) {
		//Gdx.app.log(TAG, "update " + reversed);
		time += dt;
		...
		if (!jumping && parent.containsAction(AWallSlide.class)){
		...
		if (jumping){
			...
			if (
				...
				Gdx.app.log(TAG,  "stop jumping (height)");
		...
		if (jumpTime > conn.cost + .05f){
			isFinished = true;
			Gdx.app.log(TAG,  "time over STOP" + conn.cost + " : " + parent.engine.tick);
		}
		//float dx = conn.to.x + .5f - pos.x;
		if ((conn.key & APathfindingJumpAndHold.WALLJUMP) != 0){
			if (hasJumped){
				reversed = true;
			}
		}
...
//		}
	...
	public void onStart() {
		...
		heightGoal = APathfindingJumpAndHold.HEIGHT_GOAL[index] + (int)pos.y + .5f;
		startHeight = pos.y;
